...
<ul>
<?php  
$exclude_id = $post->ID;$posttags = get_the_tags();$i = 0;$limit = !dopt('d_related_count') ? 12 : dopt('d_related_count');;if ( $posttags ){$tags = '';foreach ( $posttags as $tag ) $tags .= $tag->name . ',';$args = array('post_status' => 'publish','tag_slug__in' => explode(',',$tags),'post__not_in' => explode(',',$exclude_id),'caller_get_posts' => 1,'orderby' => 'comment_date','posts_per_page' => $limit

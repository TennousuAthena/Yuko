...
class d_postlist extends WP_Widget {
	function d_postlist() {
        ...
        parent::__construct( 'd_postlist', 'Yuko 聚合文章', $widget_ops, $control_ops );
	}
...
?>
<li><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" ><?php if( $img ){echo '<span class="thumbnail">';echo '<img src="'.get_bloginfo("template_url").'/timthumb.php?src=';echo post_thumbnail_src();echo '&h=64&w=100&q=90&zc=1&ct=1" alt="'.get_the_title().'" /></span>'; }else{$img = '';} ?><span class="text"><?php the_title(); ?></span><span class="muted"><?php the_time('Y-m-d');?></span><span class="muted" style="float: right;"><?php comments_number('', '1评论', '%评论'); ?></span></a></li>
<?php

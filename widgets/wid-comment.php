...
function mod_newcomments( $limit,$outpost,$outer ){
	...
	foreach ( $comments as $comment ) {
		$output .= '<li><a href="'.get_permalink($comment->ID).'#comment-'.$comment->comment_ID.'" title="'.$comment->post_title.'上的评论">'.get_avatar( $comment->comment_author_email, $size = '48' , deel_avatar_default()).' <div class="muted"><i>'.strip_tags($comment->comment_author).'</i>'.timeago( $comment->comment_date ).'说：'.convert_smilies(strip_tags($comment->com_excerpt)).'</div></a></li>';
	}
